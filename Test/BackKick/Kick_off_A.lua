...
["start1"] = {
	switch = function()
		if Cbuf_cnt(CBall2RoleDist("Kicker")<35,10) and CRole2TargetDist("Receiver")<3 then
			return "passball"
		end
	end,
	Kicker  = task.KickerTask("getBall"),
...
["receive"] = {
	switch = function()
		if Cbuf_cnt(CBall2RoleDist("Receiver")<20,10) then
			return "shoot"
		end
	end,
	Kicker   = task.KickerTask("canShoot1"),--GotoPos("Kicker",RobotPosX("Kicker"),RobotPosY("Kicker"),Receiver2Balldir()),
...
["shoot"] = {
	...
	Kicker   = task.KickerTask("canShoot1"),
